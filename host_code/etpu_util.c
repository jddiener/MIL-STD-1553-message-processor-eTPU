/*lint --e{*} Suppress all lint warnings for this function */
/*******************************************************************************
*
  ...
  uint32_t globals_size)
{
  ...
  fs_etpu_free_param = fs_memcpy32((uint32_t*)fs_etpu_data_ram_start, globals, globals_size);
  fs_etpu_free_param = (uint32_t*)((((uint32_t)fs_etpu_free_param + 7) >> 3) << 3); /* round up to 8s */
  ...
  uint32_t engine_mem_size)
{
  ...
  if(engine_mem_size > 0)
  {
    ...
    if(eTPU->ECR_A.B.MDIS == 0)
    {
      fs_etpu_free_param = (uint32_t*)((((uint32_t)fs_etpu_free_param+511)>>9)<<9); /* round up to 512s */
      eTPU->ECR_A.B.ERBA = ((uint32_t)fs_etpu_free_param) >> 9;
      fs_memset32(fs_etpu_free_param, 0, engine_mem_size);
      fs_etpu_free_param = (uint32_t*)((uint32_t)fs_etpu_free_param + engine_mem_size);
    }
    /* Engine B */
    if(eTPU->ECR_B.B.MDIS == 0)
    {
      fs_etpu_free_param = (uint32_t*)((((uint32_t)fs_etpu_free_param+511)>>9)<<9); /* round up to 512s */
      eTPU->ECR_B.B.ERBA = ((uint32_t)fs_etpu_free_param) >> 9;
      fs_memset32(fs_etpu_free_param, 0, engine_mem_size);
      fs_etpu_free_param = (uint32_t*)((uint32_t)fs_etpu_free_param + engine_mem_size);
    }
  }
  ...
  if ((uint32_t)fs_etpu_free_param > fs_etpu_data_ram_end)
  ...
  uint32_t *func_frame)
{
  ...
  eTPU->CHAN[channel].CR.R = config + (function<<16) + (((uint32_t)func_frame - fs_etpu_data_ram_start)>>3);
...
uint32_t *fs_etpu_get_cpba_pse(
  uint8_t channel)
{
  return((uint32_t*)((uint8_t*)fs_etpu_data_ram_ext + (eTPU->CHAN[channel].CR.B.CPBA << 3)));
}
  ...
  uint16_t num_bytes)
{
  ...
  pba = fs_etpu_free_param;
  fs_etpu_free_param += (((num_bytes+7)>>3)<<1);

  if(fs_etpu_free_param > (uint32_t *)fs_etpu_data_ram_end)
    return(0);
  else
    return(pba);
}
...
uint24_t fs_etpu_get_chan_local_24(
  ...
  uint32_t offset)
{
  return(0x00FFFFFF & (*(uint32_t *)((uint32_t)fs_etpu_data_ram_start + (eTPU->CHAN[channel].CR.B.CPBA<<3) + offset-1)));
}
...
uint24_t fs_etpu_get_global_24(
  uint32_t offset)
{
  return(0x00FFFFFF & (*(uint32_t *)((uint32_t)fs_etpu_data_ram_start + offset-1)));
}
...
*******************************************************************************/
uint8_t fs_etpu_get_chan_interrupt_oveflow_flag(
  uint8_t channel)
{
...
*******************************************************************************/
uint8_t fs_etpu_get_chan_dma_oveflow_flag(
  uint8_t channel)
{
...
uint32_t fs_etpu_get_global_error(void)
{
  ...
  return(*(uint32_t *)((uint32_t)fs_etpu_data_ram_start + FS_ETPU_OFFSET_GLOBAL_ERROR));
#else /* presume the Global Error is at address 0, which is the case of all set1-set4 */
  return(*(uint32_t *)((uint32_t)fs_etpu_data_ram_start));
#endif
}
  ...
  int32_t *value2)
{
  ...
  if ((uint32_t)fs_etpu_free_param + 8 > fs_etpu_data_ram_end)
  {
  ...
  else
  {
    ...
    else
    {
      ...
      addr_b = ((uint32_t)fs_etpu_free_param - fs_etpu_data_ram_start) >> 3;
      ...
      *value1 = ((*(fs_etpu_free_param))<<8)>>8;
      *value2 = ((*(fs_etpu_free_param + 1))<<8)>>8;
    }
  }
  return(err_code);
}
  ...
  uint32_t *value2)
{
  ...
  else
  {
    ...
    else
    {
      ...
      /* read values from temporary buffer */
      *value1 = *(fs_etpu_free_param);
      *value2 = *(fs_etpu_free_param + 1);
    }
  }
  return(err_code);
}
  ...
  int32_t value2)
{
  ...
  else
  {
    ...
    *(fs_etpu_free_param) = value1;
    *(fs_etpu_free_param + 1) = value2;
